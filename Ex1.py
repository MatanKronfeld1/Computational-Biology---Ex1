...
class CellularAutomaton:
    ...
    # Functio to calculate the next generation of the grid, considering wraparound mode
    ...
    # se the initial pattern (random or special) for the grid
...
# Class responsible for the gui of the animation
...
class AutomatonGUI:
    def __init__(self, root):
        ...
        self.animation_container = ttk.Frame(self.container)
        self.animation_container.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
    ...
    def setup_sidebar(self):
        ...
        self.buttons_container = ttk.Frame(self.sidebar)
        self.buttons_container.pack(fill=tk.X, pady=5)

        self.run_one_button = ttk.Button(self.buttons_container, text="Run One Generation", 
                                      command=self.run_one_generation)
        self.run_one_button.pack(fill=tk.X, pady=5)
        
        self.start_button = ttk.Button(self.buttons_container, text="Start Simulation", 
                                  command=self.start_simulation)
        self.start_button.pack(fill=tk.X, pady=5)
        
        self.stop_button = ttk.Button(self.buttons_container, text="Stop Simulation", 
                              command=self.stop_animation)
        self.stop_button.pack(fill=tk.X, pady=5)
        self.stop_button.config(state='disabled')
        
        self.resume_button = ttk.Button(self.buttons_container, text="Resume Simulation", 
                               command=self.resume_simulation)
        self.resume_button.pack(fill=tk.X, pady=5)
        self.resume_button.config(state='disabled')
        
        self.reset_button = ttk.Button(self.buttons_container, text="Reset All", 
        ...
        self.unchanged = tk.StringVar(value="Unchanged: 0.00%")
        self.living_cell = tk.StringVar(value="Living Cells: 0.00%")
        self.std_d = tk.StringVar(value="Std Dev (Blocks): 0.00")
        ttk.Label(self.sidebar, textvariable=self.unchanged).pack(pady=5, fill=tk.X)
        ttk.Label(self.sidebar, textvariable=self.living_cell).pack(pady=5, fill=tk.X)
        ttk.Label(self.sidebar, textvariable=self.std_d).pack(pady=5, fill=tk.X)

        # a line separating the sidebar from the animation
    def setup_simulation_area(self):
        self.plot_frame = ttk.Frame(self.animation_container)
    ...
    # change speed according to user's preference
    ...
    # reset animation for starting over
    ...
    def start_simulation(self):
        ...
        threading.Thread(target=self.run_animation, args=(grid_size, generations, live_prob, wraparound, pattern)).start()
    ...
    def stop_animation(self):
        if self.anim:
            ...
            self.run_one_button.config(state='normal')
            self.message_var.set("Simulation paused. Click 'Resume Simulation' to continue. Speed an generations can be changed.")
    ...
    def resume_simulation(self):
        ...
        if self.ca.generation >= total_generations:
            self.message_var.set(f"Cannot resume. Generation limit reached: {total_generations}.")
            return
        ...
        self.message_var.set("Running...")

        remaining_generations = total_generations - self.ca.generation
        ...
        def update(frame):
            ...
            if is_first_frame_after_resume:
                ...
                self.ca.next_generation(update_gui_callback=lambda percentage_unchanged, percentage_living, std_dev_blocks: (
                    self.unchanged.set(f"Unchanged: {percentage_unchanged:.2f}%"),
                    self.living_cell.set(f"Living Cells: {percentage_living:.2f}%"),
                    self.std_d.set(f"Standard Deviation: {std_dev_blocks:.2f}")
                ))
    ...
    def run_next_generation(self):
        total_generations = int(self.gen_var.get())
        if self.ca.generation >= total_generations:
            self.message_var.set(f"Generation limit reached: {total_generations}.")
            return
    ...
    def run_animation(self, grid_size, generations, live_prob, wraparound, pattern):
      self.simulation_running = True
